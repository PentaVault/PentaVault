...
| KMS | Envelope encryption abstraction; an external-KMS provider that fails closed with no local fallback; concrete **AWS KMS, Google Cloud KMS and Azure Key Vault** clients behind one `ExternalKmsClient` seam — AWS binds each wrapped key with an encryption context, GCP with `additionalAuthenticatedData` and a self-signed service-account JWT rather than a token exchange, and Azure through `wrapKey`/`unwrapKey` on a pinned RSA key version (RSA-OAEP has no authenticated-data equivalent, so the version pin plus the provider/keyRef on every envelope is the binding). GCP and Azure call the REST APIs directly, keeping two cloud SDK dependency trees out of the package that handles key material; a key registry that routes unwrapping by the provider and key reference on each envelope, including keys resolved at runtime so adopting one needs no restart; **per-organisation BYOK** (owner-only, verified by a wrap/unwrap probe before storing, retired rather than deleted so no data is stranded); and a resumable re-wrap migration covering every envelope-bearing table — secret versions, app connections, dynamic secret leases, webhooks, external shares and LDAP bind passwords — that moves data onto a new key without ever decrypting a value | A KMIP appliance adapter; per-organisation BYOK for GCP and Azure (which needs somewhere to keep customer cloud credentials, a problem the AWS path sidesteps by using the ambient credential chain); running the re-wrap automatically on adoption rather than by request | Partial |
...
Per-token `allowedIps` now runs through the same matcher, so a token can be
pinned to a range and an IPv4-mapped source matches a plain IPv4 entry. Entries
are canonicalised at issue time, so a bare address is stored as `/32`.

Still outstanding: the *organisation* allowlist is not applied to proxy
(`pv_tok_`) or machine-identity (`pv_mid_`) tokens. Neither carries a session
cookie, so the request guard never sees them, and resolving the owning
organisation on the proxy hot path is a larger change than the matcher
unification.
