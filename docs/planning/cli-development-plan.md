...
1. `pv login` opens browser with Authorization Code + PKCE.
2. If browser is unavailable, use device code login.
3. CLI receives refresh token or device credential.
4. Store persistent credential in the OS credential store only.
5. Keep access tokens short-lived and in memory where possible.
